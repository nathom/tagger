...
parser.add_argument(
    ...
    "--pattern",
    nargs="?",
    help="pattern of track names e.g. <track> - <artist>.flac",
    default=None,
)
parser.add_argument(
    "-i",
    "--ignore-parens",
    help="ignore any text in parentheses when matching",
    action="store_true",
)
...
while unsatisfied:
    resp = input(
        f'Press enter to continue. Type "n" to get next result. Type "s" to switch engines. Type anything else to manual search.\n'
    )
    if resp == "n":
        album.next()
        match(album, path, pattern)
